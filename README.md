...
| [ankra cluster](https://docs.ankra.ai/reference/cli/cluster) | Cluster operations: stacks, manifests, addons, operations, variables, access, SOPS encryption, agent status, upgrade, the auto-upgrade opt-out (`cluster agent auto-upgrade enable\|disable` - fence an agent off from the fleet rollout for a freeze window; `agent upgrade` still applies a release on demand) and pipeline-step CI settings (`cluster agent ci get\|set` - how many Ankra Pipelines steps the cluster's agent runs at once and the storage class their workspaces use, stored on the platform so an agent upgrade no longer renders the setting away), provider (Hetzner/OVH/UpCloud/DigitalOcean) lifecycle |
...
| [ankra registry](https://docs.ankra.ai/reference/cli/registry) | Manage the organisation's Ankra registry: create, list, get, rotate and revoke robot accounts on its project (`registry robots create <name> --scope push\|pull` prints the secret once, with the `docker login` that uses it; the login is stored as the managed credential `ankra-harbor-robot-<name>`) |
