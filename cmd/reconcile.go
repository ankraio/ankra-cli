...
If no cluster name is provided, uses the currently selected cluster.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// The Hetzner, UpCloud, OVH, DigitalOcean and AWS deprovision
		// endpoints honor force; the Proxmox/Morpheus lanes and the generic
		// imported deprovision drop it, so say so instead of implying a
		...
		if force {
			...
			case cloudClusterKindHetzner, cloudClusterKindUpcloud,
				cloudClusterKindOvh, cloudClusterKindDigitalocean, cloudClusterKindAws:
			default:
				_, _ = fmt.Fprintln(cmd.ErrOrStderr(),
...
func init() {
	...
	clusterDeprovisionCmd.Flags().Bool("force", false, "Force deprovision even if cluster is in an unexpected state; on UpCloud, Hetzner, OVH, DigitalOcean and AWS also deletes leftover CSI storage volumes and load balancers")
