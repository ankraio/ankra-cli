...
// The 32-case contract table that used to live here has been replaced by the
// shared conformance corpus in testdata/corpus.json, asserted by
// TestContractCorpus in corpus_test.go (ankra-4r75g.11). Every vector it held
// is covered by a canonical case except two, which moved to
// TestRepoLocalExtraCases with the reason they are local.
//
// Add a vector to the canonical corpus, not here: the whole point is that six
// implementations in five repositories assert the same file.
